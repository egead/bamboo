...
path_resamp = "/media/ege/DATA/FULLCHINA/LastResamp_1Min/"
    ...
    for root, dirs, files in walk(base_path.format(day)+"/"):
        for file in files:
            if "checkpoint" in file:
                ...
                df_resample = df.resample('T').last()
                df_resample.to_csv(path_resamp+day+"/"+file)

#5 Minute Time Windows
path_resamp5 = "/media/ege/DATA/FULLCHINA/LastResamp_5Min/"
    ...
    for root, dirs, files in walk(base_path.format(day)+"/"):
        for file in files:
            if "checkpoint" in file:
                ...
                df.set_index('ts',inplace=True)
                df = df.loc[:, ['ltp',"trade_size","level_qty:ask:0","level_qty:bid:0","level_price:ask:0"]]
                df_resample = df.resample('5T').last()
                df_resample.to_csv(path_resamp5+day+"/"+file)

#10 Minute Time Windows
path_resamp10 = "/media/ege/DATA/FULLCHINA/LastResamp_10Mins/"
if not exists(path_resamp10):
    mkdir(path_resamp10)

for day in day_paths:
    if not exists(path_resamp10+day+"/"):
    ...
    for root, dirs, files in walk(base_path.format(day)+"/"):
        for file in files:
            if "checkpoint" in file:
                ...
                df.set_index('ts',inplace=True)
                df = df.loc[:, ['ltp',"trade_size","level_qty:ask:0","level_qty:bid:0","level_price:ask:0"]]
                df_resample = df.resample('10T').last()
